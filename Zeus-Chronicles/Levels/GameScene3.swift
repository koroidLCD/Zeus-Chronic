...
class GameScene3: SKScene {
    ...
    func performAnimation() {
            let imageSprite = SKSpriteNode(imageNamed: "zeus")
            imageSprite.size = CGSize(width: frame.width, height: frame.height)
            imageSprite.position = CGPoint(x: frame.midX, y: 0)//frame.midY)
            imageSprite.alpha = 0.0
    ...
    private func spawnShape() {
        ...
        if randomColor == "symb_Y" || randomColor == "symb_Y2" {
            shape.name = "green"
        } else {
            shape.name = "red"
        }
    ...
    private func spawnTwoShapes() {
        ...
        addChild(shape2)
        if colors.first == "symb_Y" || colors.first == "symb_Y2"{
            shape1.name = "green"
            shape2.name = "red"
        } else {
            shape1.name = "red"
        ...
        if !isGameOver {
            for touch in touches {
                ...
                if !isHard {
                    ...
                    enumerateChildNodes(withName: "green") { (node, _) in
                        if let shape = node as? SKSpriteNode {
                            if shape.position.x < self.size.width / 2 {
                                shape.run(moveRightAction)
                            } else {
                                shape.run(moveLeftAction)
                            }
                        }
                    }
                    enumerateChildNodes(withName: "red") { (node, _) in
    ...
    private func handleCollision(with shape: SKSpriteNode) {
        ...
        if gameDelegate.score % 10 == 0 {
            performAnimation()
            isHard.toggle()
            if colors.first == "symb_R" || colors.first == "symb_Y" {
                colors = ["symb_R2", "symb_Y2"]
            } else {
                colors = ["symb_R", "symb_Y"]
            }
